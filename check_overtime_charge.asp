...
'
' ## プログラム概要 ##
' 閲覧者（給与担当者）の管理組織に属する職員の、時間外状況を月別に閲覧するページです。
'
' （機能）
...
<div id="contents">
    <p>
        時間外確認　
        <a href="check_overtime_charge.asp?ymb=<%=prevYmb%>">&lt;&lt;</a>&nbsp;
        <%=dispYear%>年<%=dispMonth%>月&nbsp;
        <a href="check_overtime_charge.asp?ymb=<%=nextYmb%>">&gt;&gt;</a>
                        ...
                        For i = 1 To dispLastDay
                            ...
                            Else
                                If sumOvertime = "0" Then
                                    sumOvertime = "&nbsp;"
                                'Else
                                '    sumOvertime = FormatNumber(val(sumOvertime), 0, 0, -1)
                                End If
                            End If
